...
function Register-EventHandlers {
    ...
    # VM Settings tab event handlers (Hypervisor selection)
    if ($null -ne $State.Controls.cmbHypervisorType) {
        $State.Controls.cmbHypervisorType.Add_SelectionChanged({
            param($eventSource, $selectionChangedEventArgs)
            $window = [System.Windows.Window]::GetWindow($eventSource)
            $localState = $window.Tag

            $selectedIndex = $eventSource.SelectedIndex
            # Update hypervisor status and show/hide Hyper-V specific controls
            Update-HypervisorStatus -State $localState -SelectedIndex $selectedIndex
        })
    }

    # VMware REST API credential configuration button handler
    if ($null -ne $State.Controls.btnConfigureVMrest) {
        $State.Controls.btnConfigureVMrest.Add_Click({
            param($eventSource, $routedEventArgs)
            $window = [System.Windows.Window]::GetWindow($eventSource)
            $localState = $window.Tag

            try {
                WriteLog "Opening terminal to configure vmrest credentials..."

                # Find vmrest.exe path
                $vmrestPath = $null
                $regPaths = @(
                ...
                if (-not $vmrestPath -or -not (Test-Path $vmrestPath)) {
                    [System.Windows.MessageBox]::Show(
                        "VMware Workstation vmrest.exe not found.`n`nPlease ensure VMware Workstation Pro is installed.",
                        "VMware Not Found",
                ...
                # Open a new terminal window with vmrest -C command (non-elevated)
                # Note: We don't request elevation here. If your organization uses separate
                # admin accounts for elevation, you should run vmrest.exe -C from an elevated
                # command prompt manually to configure credentials for the build account.
                $psi = New-Object System.Diagnostics.ProcessStartInfo
                $psi.FileName = 'cmd.exe'
                $psi.Arguments = "/K echo. && echo ============================================= && echo  VMware REST API Credential Configuration && echo ============================================= && echo. && echo This will configure credentials for the vmrest.exe REST API. && echo Enter a username and password when prompted. && echo. && echo Note: Credentials are stored in your user profile at: && echo   %USERPROFILE%\.vmrestCfg && echo. && echo IMPORTANT: If your FFU builds run with a separate admin account, && echo you must ALSO run vmrest.exe -C from an elevated command prompt && echo to configure credentials for that account. && echo. && echo Press Ctrl+C to cancel, or proceed below: && echo. && `"$vmrestPath`" -C && echo. && echo Configuration complete! You can close this window. && pause"
                $psi.UseShellExecute = $true
                # Note: Removed 'runas' elevation - credentials stored per-user profile

                [System.Diagnostics.Process]::Start($psi) | Out-Null

                WriteLog "Terminal opened for vmrest credential configuration (non-elevated)"

                # Show info message with enterprise guidance
                [System.Windows.MessageBox]::Show(
                    "A terminal window has been opened to configure vmrest credentials.`n`nFollow the prompts in the terminal to set up your username and password.`n`nIMPORTANT: If your builds run with a separate admin account (common in enterprise environments), you must also configure credentials for that account by running 'vmrest.exe -C' from an elevated command prompt.`n`nAfter configuration is complete, click OK to refresh the status.",
                    "Configure VMware REST API",
                    [System.Windows.MessageBoxButton]::OK,
                    [System.Windows.MessageBoxImage]::Information
                )

                # Refresh status after configuration
                Update-HypervisorStatus -State $localState -SelectedIndex $localState.Controls.cmbHypervisorType.SelectedIndex
            }
            catch {
                WriteLog "Error opening vmrest configuration: $($_.Exception.Message)"
                [System.Windows.MessageBox]::Show(
                    "Failed to open vmrest configuration: $($_.Exception.Message)",
                    "Error",
                    [System.Windows.MessageBoxButton]::OK,
    ...
    # VM Switch selection handler (Hyper-V specific)
